...
To run the environment enumeration function, type "enumerate_env()".
...
def enumerate_env():
    env_vars = os.environ
    env_string = "\n".join([f"{key}: {env_vars[key]}" for key in env_vars])
    agent.memorize(f"Environment Variables:\n{env_string}\n")
